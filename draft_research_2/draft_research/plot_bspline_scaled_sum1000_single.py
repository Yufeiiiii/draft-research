...
def main():
    ...
    curve_specs = [
        (
            "GP",
            PROJECT_DIR / "results" / "gp_sum" / "draft_curve_gp_sum_lookup.csv",
            "#d95f02",
        ),
        (
            "PS",
            PROJECT_DIR / "results" / "ps_sum" / "draft_curve_ps_sum_lookup.csv",
            "#1b9e77",
        ),
        (
            "TOI",
            ...
            / "results"
            / "toi_sum_min"
            / "draft_curve_toi_sum_min_lookup.csv",
            "#7570b3",
        ),
    ]
    ...
    ax.set_xlim(1, int(combined_df["overall"].max()))
    ax.legend(loc="upper right", frameon=False, title="Metric")

    plt.tight_layout()

    csv_output_path = RESULTS_DIR / "draft_curve_bspline_scaled_sum1000_single.csv"
    plot_output_path = RESULTS_DIR / "draft_curve_bspline_scaled_sum1000_single.png"
