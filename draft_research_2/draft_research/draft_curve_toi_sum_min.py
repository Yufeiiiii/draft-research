...
TARGET_COLUMN = "TOI_sum_min"
PREDICTOR_COLUMN = "Selection"
GROUP_COLUMN = "Year"
TRAIN_MAX_PICK = 220
PREDICTION_MAX_PICK = 224
...
def compute_sigma_by_pick(train_df, fallback=1.0):
    sigma_by_pick = train_df.groupby(PREDICTOR_COLUMN)["response_raw"].std()
    sigma_by_pick = sigma_by_pick.replace([np.inf, -np.inf], np.nan).fillna(fallback)
    return sigma_by_pick.to_dict()


def get_selection_weights(selection_values, sigma_by_pick, fallback=1.0):
    weights = []
    for selection in selection_values:
        weight = sigma_by_pick.get(selection, fallback)
        # Fallback to 1.0 when a pick-level sd is missing or non-finite.
...
def compute_weighted_rmse(y_true, y_pred, weights):
...
def build_curve_from_idata(idata, design_info, max_pick):
    ...
    mu_floor_5pct = np.percentile(mu_grid_samples, 5, axis=0)
    mu_ceiling_95pct = np.percentile(mu_grid_samples, 95, axis=0)

    pick_1_value = mu_mean[0]
    if pick_1_value > 0:
        expected_value = 100 * mu_mean / pick_1_value
        value_floor_5pct = 100 * mu_floor_5pct / pick_1_value
        value_ceiling_95pct = 100 * mu_ceiling_95pct / pick_1_value
    else:
        expected_value = mu_mean
        value_floor_5pct = mu_floor_5pct
        value_ceiling_95pct = mu_ceiling_95pct

    return pd.DataFrame(
        {
            "overall": pick_grid.astype(int),
            "expected_draft_value": expected_value,
            "value_floor_5pct": value_floor_5pct,
            "value_ceiling_95pct": value_ceiling_95pct,
            "raw_model_output": mu_mean,
            "raw_floor_5pct": mu_floor_5pct,
            "raw_ceiling_95pct": mu_ceiling_95pct,
        }
    )


def format_value_interval(value, lower, upper):
    def format_piece(piece):
        return "NA" if pd.isna(piece) else f"{piece:.2f}"

    if pd.isna(value) and pd.isna(lower) and pd.isna(upper):
        return ""
    return (
        f"{format_piece(value)} "
        f"({format_piece(lower)}, {format_piece(upper)})"
    )


def build_value_table(curve_df):
    value_table = pd.DataFrame(
        {
            "Pick": curve_df["overall"].astype(int),
            "V": curve_df["expected_draft_value"],
            "L": curve_df["value_floor_5pct"],
            "U": curve_df["value_ceiling_95pct"],
        }
    )
    value_table["V_L_U"] = [
        format_value_interval(value, lower, upper)
        for value, lower, upper in zip(
            value_table["V"], value_table["L"], value_table["U"]
        )
    ]
    return value_table


def save_value_table_latex(value_table, tex_path, columns_per_row=4):
    cells = []
    for row in value_table.itertuples(index=False):
        pick_text = "" if pd.isna(row.Pick) else str(int(row.Pick))
        value_text = row.V_L_U if row.V_L_U else "NA"
        cells.append(f"\\shortstack[c]{{{pick_text}\\\\{value_text}}}")

    while len(cells) % columns_per_row != 0:
        cells.append("~")
    ...
    lines = [
        ...
        "\\centering",
        "\\scriptsize",
        f"\\begin{{tabular}}{{{'c' * columns_per_row}}}",
        "\\hline",
    ]

    for start in range(0, len(cells), columns_per_row):
        row_cells = cells[start : start + columns_per_row]
...
def fit_monotone_spline_fold(
    ...
    selection_test = test_df[PREDICTOR_COLUMN].values.astype(int)
    ...
    sigma_by_pick = compute_sigma_by_pick(train_df)
    test_weights = get_selection_weights(selection_test, sigma_by_pick, fallback=1.0)
    ...
    weighted_rmse_normalized = compute_weighted_rmse(y_test, y_pred, test_weights)
    rmse_raw = np.sqrt(np.mean((y_test_raw - y_pred_raw) ** 2))
    weighted_rmse_raw = compute_weighted_rmse(y_test_raw, y_pred_raw, test_weights)
...
def save_curve_plot(df_train, master_curve, export_dir, target_column):
    ...
    plt.fill_between(
        master_curve["overall"],
        master_curve["raw_floor_5pct"],
        master_curve["raw_ceiling_95pct"],
        color="red",
        alpha=0.15,
...
def main():
    ...
    spline_formula = get_spline_formula(PREDICTION_MAX_PICK)
    ...
    extended_curve = build_curve_from_idata(idata, design_info, PREDICTION_MAX_PICK)
    extended_curve_path = (
        export_dir
        / f"draft_curve_{TARGET_COLUMN.lower()}_lookup_extended_{PREDICTION_MAX_PICK}.csv"
    )
    extended_curve.to_csv(extended_curve_path, index=False)

    value_table = build_value_table(extended_curve)
    value_table_path = (
        export_dir
        / f"draft_curve_{TARGET_COLUMN.lower()}_value_table_{PREDICTION_MAX_PICK}.csv"
    )
    value_table.to_csv(value_table_path, index=False)

    value_table_tex_path = (
        export_dir
        / f"draft_curve_{TARGET_COLUMN.lower()}_value_table_{PREDICTION_MAX_PICK}.tex"
    )
    save_value_table_latex(value_table, value_table_tex_path)
    ...
    print(f"Saved extended curve lookup: {extended_curve_path}")
    print(f"Saved value table CSV: {value_table_path}")
    print(f"Saved value table LaTeX: {value_table_tex_path}")
