...
def load_garsum_scatter():
    data_path = ROOT_DIR / "draft_pick_wide_GAR_adjusted_2009_2018_GARsum1_7.csv"
    df = pd.read_csv(data_path)
    df = df[["DraftOv", "GARsum1_7"]].copy()
    ...
    target_min = df["GARsum1_7"].min()
    target_max = df["GARsum1_7"].max()
    df["response_normalized"] = df["GARsum1_7"].apply(
        lambda x: normalize_to_1_100(x, target_min, target_max)
    )
    return df
...
def plot_panel(ax, scatter_df, x_col, curve_path, title, xlabel, xlim_max):
    ...
    ax.fill_between(
        curve_df["overall"],
        curve_df["raw_floor_3pct"],
        curve_df["raw_ceiling_97pct"],
        color="red",
        alpha=0.15,
    )
...
def main():
    ...
    garsum_scatter = load_garsum_scatter()
    ...
    plot_panel(
        ...
        title="Draft Curve for TOI_sum_min",
        xlabel="Draft Pick (Selection)",
        xlim_max=215,
    )
    plot_panel(
        ax=axes[3],
        scatter_df=garsum_scatter,
        x_col="DraftOv",
        curve_path=ROOT_DIR / "results" / "garsum1_7_curve" / "draft_curve_garsum1_7_lookup.csv",
        title="Draft Curve for GARsum1_7",
        xlabel="Draft Pick (Overall)",
        xlim_max=int(garsum_scatter["DraftOv"].max()) + 5,
    )

    fig.suptitle("Draft Curve Comparison", fontsize=16, fontweight="bold")
